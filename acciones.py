...
def aplicar_setpoint(valor: float) -> str:
    ...
    Args:
        valor: potencia de referencia en W (rango 0–15000).

    Returns:
        Mensaje de confirmación para mostrar en la UI.
    """
    valor = float(valor)
    if not (0 <= valor <= 15000):
        return "✗ Valor fuera de rango (0–15000 W)."
    datos.set_setpoint(valor)
    return f"✓ Setpoint de {valor} W aplicado."
