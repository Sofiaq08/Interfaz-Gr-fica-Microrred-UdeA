...
_estados = {
    ...
    "setpoint_potencia_w":  0.0,   # float  — potencia de referencia del inversor
}
...
def set_setpoint(valor: float) -> None:
    ...
    if not (0.0 <= valor <= 15000.0):
        raise ValueError(f"Setpoint fuera de rango [0, 15000] W: {valor}")
    ...
    try:
        ...
        punto = Point(MEASUREMENT_CTRL) \
            .field("setpoint_potencia_w", float(valor)) \
            .time(datetime.now(timezone.utc))

        write_api.write(bucket=INFLUX_BUCKET, org=INFLUX_ORG, record=punto)
        client.close()

        _estados["setpoint_potencia_w"] = float(valor)
        _guardar_estados()
